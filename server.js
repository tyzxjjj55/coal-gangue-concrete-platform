...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>工作区 - ${html(content.siteTitle)}</title>
...
<body>
  <header>
    <div class="bar">
      <h1>工作区</h1>
      <div><a href="/" target="_blank" rel="noopener">首页</a> · <a href="${BASE_PATH}/" target="_blank" rel="noopener">后台</a> · <a href="/gallery/" target="_blank" rel="noopener">相册</a> · <a href="${WORK_PATH}/logout">退出</a></div>
    </div>
  </header>
  <main>
    ${message ? `<p class="message">${html(message)}</p>` : ""}
...
function renderTestBlocksPanel(content, options = {}) {
  ...
  return `<section class="panel" id="test-blocks">
      ...
      <form class="compact-form" method="post" action="${actionBase}/add-block">
        <div class="grid">
          <label>试块名称/部位<input name="blockName" placeholder="例：1#楼三层梁板 C30" required></label>
          <label>制作日期<input name="madeDate" type="date" value="${today()}" required></label>
        </div>
        <div class="grid">
          <label>试块数量<input name="quantity" type="number" min="1" step="1" value="3" required></label>
          <label>需要龄期<input name="ages" value="7,28" placeholder="例：3,7,28"></label>
        </div>
        <div class="grid">
          <label>强度等级<input name="strength" placeholder="例：C30"></label>
          <label>备注<input name="note" placeholder="可填编号、施工段等"></label>
        </div>
        <div class="grid">
          <label>配方/编号<input name="mixName" placeholder="例：C30 泵送配合比"></label>
          <label>水胶比<input name="waterBinderRatio" placeholder="例：0.42"></label>
        </div>
        <label>坍落度<input name="slump" placeholder="例：180mm"></label>
        <p class="hint">拆模提醒固定为制作日期后 1 天；龄期到期按“制作日期 + 龄期天数”计算。多个龄期用逗号分开，例如 7,28,56。</p>
        <div class="actions"><button type="submit">登记试块</button></div>
      </form>
      ${options.showMailAction ? `<form class="compact-form" method="post" action="${actionBase}/send-reminder"><h2>邮件提醒</h2><p class="hint">每天 08:00 后会自动发送今日和逾期提醒。这里可以手动发送一封当前提醒摘要。</p><div class="actions"><button class="secondary" type="submit">立即发送提醒邮件</button></div></form>` : ""}
...
function renderReminderSummary(content) {
  ...
  return `<div class="summary-grid">
        ${renderReminderCard("今日拆模", demold.today, "demold", WORK_PATH)}
        ${renderReminderCard("逾期未处理", [...demold.overdue, ...ages.overdue], "overdue", WORK_PATH)}
        ${renderReminderCard("未来 7 天", [...demold.upcoming, ...ages.upcoming], "upcoming", WORK_PATH)}
      </div>
      <div class="summary-grid">
        ${renderReminderCard("今日龄期", ages.today, "age", WORK_PATH)}
        ${renderReminderCard("逾期龄期", ages.overdue, "overdue", WORK_PATH)}
        ${renderReminderCard("未来龄期", ages.upcoming, "upcoming", WORK_PATH)}
      </div>`;
}

function renderReminderCard(title, items, kind, actionBase) {
  return `<section class="summary-card${kind === "overdue" ? " overdue" : ""}">
        <strong>${html(title)}（${items.length}）</strong>
        ${items.length ? `<div class="task-list">${items.map((item) => renderTaskCheckbox(item, actionBase)).join("")}</div>` : `<p class="hint">暂无</p>`}
      </section>`;
}
...
function blockRecordSummary(record) {
  const data = normalizeBlockRecord(record);
  const parts = [];
  if (data.mixName) parts.push(`配方：${data.mixName}`);
  if (data.waterBinderRatio) parts.push(`水胶比：${data.waterBinderRatio}`);
  if (data.slump) parts.push(`坍落度：${data.slump}`);
  if (data.compressionStrength) parts.push(`抗压：${data.compressionStrength}`);
  if (data.flexuralStrength) parts.push(`抗折：${data.flexuralStrength}`);
  return parts.length ? parts.join(" · ") : "未填写配方/试验数据";
}
...
function renderBlockRecord(block, actionBase = WORK_PATH) {
  ...
  return `<details class="record-details" ${hasBlockRecord(record) ? "open" : ""}>
          <summary>试块档案 <span>${html(blockRecordSummary(record))}</span></summary>
          <form class="record-form" method="post" action="${actionBase}/update-block-record">
            ...
            <div class="record-grid">
              ${recordField("mixName", "配方/编号", record.mixName, { placeholder: "例：C30 泵送配合比" })}
              ${recordField("cement", "水泥", record.cement, { placeholder: "例：330kg 或 P.O42.5" })}
              ${recordField("water", "水", record.water, { placeholder: "例：165kg" })}
              ${recordField("sand", "砂", record.sand, { placeholder: "例：760kg" })}
              ${recordField("stone", "石子", record.stone, { placeholder: "例：1080kg" })}
              ${recordField("admixture", "外加剂", record.admixture, { placeholder: "例：6.2kg" })}
              ${recordField("flyAsh", "粉煤灰", record.flyAsh, { placeholder: "可空" })}
              ${recordField("mineralPowder", "矿粉", record.mineralPowder, { placeholder: "可空" })}
              ${recordField("waterBinderRatio", "水胶比", record.waterBinderRatio, { placeholder: "例：0.42" })}
              ${recordField("slump", "坍落度", record.slump, { placeholder: "例：180mm" })}
              ${recordField("testDate", "试验日期", record.testDate, { type: "date" })}
              ${recordField("compressionStrength", "抗压强度", record.compressionStrength, { placeholder: "例：36.8MPa" })}
              ${recordField("flexuralStrength", "抗折强度", record.flexuralStrength, { placeholder: "例：5.2MPa" })}
            </div>
            ${recordArea("resultNote", "检测/破型备注", record.resultNote)}
...
function renderBlockList(blocks, actionBase = BASE_PATH) {
  if (!blocks.length) return `<p class="hint">还没有登记试块。</p>`;
  return `<h2>已登记试块</h2>
      <div class="block-list">
        ${blocks.map((block) => {
    const dueText = block.ages.map((age) => `${age}天 ${formatDateCn(addDays(block.madeDate, age))}`).join("；");
    return `<div class="block-row">
          <div>
            <strong>${html(block.name)}</strong>
            <p>制作：${formatDateCn(block.madeDate)} · 数量：${block.quantity}块${block.strength ? ` · ${html(block.strength)}` : ""}</p>
            <p>拆模：${formatDateCn(block.demoldDate || addDays(block.madeDate, 1))} · 龄期：${html(dueText)}${block.note ? ` · ${html(block.note)}` : ""}</p>
...
<body>
  <header>
    <div class="bar">
      ...
      <nav>
        <a href="/" target="_blank" rel="noopener">首页</a>
        <a href="${BASE_PATH}/" target="_blank" rel="noopener">后台</a>
        <a href="/gallery/" target="_blank" rel="noopener">相册</a>
        <a class="nav-logout" href="${WORK_PATH}/logout">退出</a>
      </nav>
    </div>
  </header>
  <main>
    ...
    <section class="workspace-hero">
      <div>
        <p class="eyebrow">试块提醒和档案</p>
        <h2>把登记、拆模、龄期和试验数据放在一个地方。</h2>
        <p>每天打开先看今日和逾期事项，做完就勾选；后面补配合比、坍落度、抗压和抗折数据，形成完整档案。</p>
      </div>
      <div class="hero-stats">
        <span><strong>${stats.blocks}</strong>试块</span>
        <span><strong>${stats.today}</strong>今日</span>
        <span><strong>${stats.overdue}</strong>逾期</span>
        <span><strong>${stats.done}</strong>已完成</span>
      </div>
    </section>
  ...
  return `<div class="panel-title">
      ...
      <div class="today-stack">
        <div class="today-group${overdueItems.length ? " warning" : ""}">
          ...
          ${overdueItems.length ? `<div class="task-list">${overdueItems.map((item) => renderTaskCheckboxV2(item, actionBase)).join("")}</div>` : `<p class="hint">没有逾期任务。</p>`}
        </div>
        <div class="today-group">
          <strong><span>今天到期</span><span>${todayItems.length}</span></strong>
          ${todayItems.length ? `<div class="task-list">${todayItems.map((item) => renderTaskCheckboxV2(item, actionBase)).join("")}</div>` : `<p class="hint">今天没有拆模或龄期任务。</p>`}
        </div>
...
function renderReminderSummaryV2(content, actionBase = WORK_PATH) {
  ...
  return `<section class="panel">
      ...
      <div class="summary-grid">
        ${renderReminderCardV2("今日拆模", demold.today, "demold", actionBase)}
        ${renderReminderCardV2("今日龄期", ages.today, "age", actionBase)}
        ${renderReminderCardV2("逾期未处理", [...demold.overdue, ...ages.overdue], "overdue", actionBase)}
      </div>
      <div class="summary-grid" style="margin-top:14px">
        ${renderReminderCardV2("未来拆模", demold.upcoming, "upcoming", actionBase)}
        ${renderReminderCardV2("未来龄期", ages.upcoming, "upcoming", actionBase)}
        ${renderReminderCardV2("全部未来 7 天", [...demold.upcoming, ...ages.upcoming], "upcoming", actionBase)}
      </div>
    </section>`;
}
...
function renderReminderCardV2(title, items, kind, actionBase) {
  return `<section class="summary-card ${kind === "overdue" ? "overdue" : ""} ${kind === "upcoming" ? "upcoming" : ""}">
        ...
        ${items.length ? `<div class="task-list">${items.map((item) => renderTaskCheckboxV2(item, actionBase)).join("")}</div>` : `<p class="hint">暂无</p>`}
      </section>`;
}
...
function renderBlockRecordV2(block, actionBase = WORK_PATH) {
  const record = normalizeBlockRecord(block.record);
  return `<details class="record-details" ${hasBlockRecord(record) ? "open" : ""}>
          <summary><span>试块档案</span><em>${html(blockRecordSummaryV2(record))}</em></summary>
          <form class="record-form" method="post" action="${actionBase}/update-block-record">
            <input type="hidden" name="id" value="${attr(block.id)}">
            <section class="record-section">
              <h4>配合比</h4>
              <div class="record-grid">
                ${recordFieldV2("mixName", "配方/编号", record.mixName, { placeholder: "例：C30 泵送配合比" })}
                ${recordFieldV2("cement", "水泥", record.cement, { placeholder: "例：330kg 或 P.O42.5" })}
                ${recordFieldV2("water", "水", record.water, { placeholder: "例：165kg" })}
                ${recordFieldV2("sand", "砂", record.sand, { placeholder: "例：760kg" })}
                ${recordFieldV2("stone", "石子", record.stone, { placeholder: "例：1080kg" })}
                ${recordFieldV2("admixture", "外加剂", record.admixture, { placeholder: "例：6.2kg" })}
                ${recordFieldV2("flyAsh", "粉煤灰", record.flyAsh, { placeholder: "可空" })}
                ${recordFieldV2("mineralPowder", "矿粉", record.mineralPowder, { placeholder: "可空" })}
                ${recordFieldV2("waterBinderRatio", "水胶比", record.waterBinderRatio, { placeholder: "例：0.42" })}
              </div>
            </section>
            <section class="record-section">
              <h4>施工与检测</h4>
              <div class="record-grid">
                ${recordFieldV2("slump", "坍落度", record.slump, { placeholder: "例：180mm" })}
                ${recordFieldV2("testDate", "试验日期", record.testDate, { type: "date" })}
                ${recordFieldV2("compressionStrength", "抗压强度", record.compressionStrength, { placeholder: "例：36.8MPa" })}
                ${recordFieldV2("flexuralStrength", "抗折强度", record.flexuralStrength, { placeholder: "例：5.2MPa" })}
                ${recordAreaV2("resultNote", "检测/破型备注", record.resultNote)}
              </div>
            </section>
            <div class="actions"><button class="secondary" type="submit">保存档案</button></div>
          </form>
...
function renderBlockListV2(blocks, actionBase = WORK_PATH) {
  ...
  return `<div class="panel-title">
        <div>
          ...
          <h2>已登记试块</h2>
          <p>点开“试块档案”补配合比、检测日期、抗压和抗折数据。</p>
        </div>
      </div>
      <div class="block-toolbar">
        <input id="blockSearch" type="search" placeholder="搜索名称、部位、强度、备注">
        <span class="hint">共 ${blocks.length} 条</span>
      </div>
      <div class="block-list">
        ${blocks.map((block) => {
    const dueText = block.ages.map((age) => `${age}天 ${formatDateCnV2(addDays(block.madeDate, age))}`).join("；");
          ...
          <div class="block-head">
            <div>
              ...
              <div class="block-meta">
                <span>制作：${formatDateCnV2(block.madeDate)}</span>
                <span>数量：${block.quantity}块</span>
                <span>拆模：${formatDateCnV2(block.demoldDate || addDays(block.madeDate, 1))}</span>
  ...
  return `<script>
    (() => {
      ...
      document.querySelectorAll("form[data-record-form]").forEach((form) => {
        ...
        form.addEventListener("submit", (event) => {
          ...
          const message = agesChanged
            ? "这个试块已经有结果：" + labels + "\\n\\n你正在修改龄期，保存后不在新龄期里的旧结果可能会被删除。确定保存吗？"
            : "这个试块已经有结果：" + labels + "\\n\\n保存会覆盖已有测试结果。确定保存吗？";
      ...
      document.querySelectorAll(".delete-gangue").forEach((button) => {
        button.addEventListener("click", (event) => {
          if (!confirm("确定删除这条煤矸石数据吗？已经关联到试块的历史名称会保留。")) event.preventDefault();
        });
      });
    })();
  </script>`;
